...
Limpiar proyecto: npm run reset-project Selecciona la opcion "no"
...
Definir Interface: Crear interface Tarea con id, texto y completada.
Estados Iniciales:
tareas: useState<Tarea[]>([])
nuevaTarea: useState("")
Función Agregar: Validar con .trim(), crear objeto con Date.now() como ID y actualizar con setTareas([...tareas, nueva]).
Input Controlado: Vincular value y onChangeText al estado nuevaTarea.
Contador de Pendientes: Calcular la longitud de las tareas no completadas.
Función Toggle: Buscar por ID y retornar un nuevo objeto con completada invertido usando .map().
Función Eliminar: Filtrar el array para excluir el ID seleccionado usando .filter().
Renderizado (FlatList):
...
Flujo de Eventos
Escritura: onChangeText actualiza el estado temporal.
Confirmación: onPress o onSubmitEditing dispara el guardado, limpia el input y re-renderiza la lista.
Interacción: Al tocar una tarea, onPress dispara toggleTarea, lo que cambia visualmente el checkbox y tacha el texto.
Borrado: Al tocar el icono de tachito, eliminarTarea remueve el item y actualiza el contador.
